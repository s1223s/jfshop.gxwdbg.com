<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:45:"./oscshop/mobile/view/agent_backend/pass.html";i:1512008342;s:37:"./oscshop/admin/view/public/base.html";i:1512008342;}*/ ?>
<!DOCTYPE html>
<html lang="en">
	<head>
...
<div class="row">
	<div class="col-xs-12">	
		<form class="form-horizontal"  id="validation-form" method="post" action='<?php echo url("AgentBackend/pass"); ?>'>
			

			
			<?php if(!isset($apply)){ ?>			
			
			<input name="aa_id" type="hidden" value="<?php echo \think\Request::instance()->param('id'); ?>" />
				...
				<div class="clearfix">
					<select  class="form-control" name="agent_level">
						<?php if(is_array($level) || $level instanceof \think\Collection || $level instanceof \think\Paginator): $i = 0; $__LIST__ = $level;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
							<option  value="<?php echo $v['al_id']; ?>"><?php echo $v['name']; ?></option>
						<?php endforeach; endif; else: echo "" ;endif; ?>
					</select>
				</div>
			</div>
			...
			<div class="form-group">
				<label class="col-sm-2 control-label no-padding-left"> 审核</label>
			<div class="col-sm-10">
				<div class="clearfix">
			...
			<div class="form-group">
				<label class="col-sm-2 control-label no-padding-left"> 未通过原因</label>
			<div class="col-sm-10">
				<div class="clearfix">
					<textarea class="form-control" rows="5" name="answer"><?php echo (isset($apply['answer']) && ($apply['answer'] !== '')?$apply['answer']:''); ?></textarea>
				</div>
			</div>
			</div>
			<div class="space-4"></div>
