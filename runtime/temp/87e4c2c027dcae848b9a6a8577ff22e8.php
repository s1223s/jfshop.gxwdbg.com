<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:40:"./oscshop/admin/view/writeoff/index.html";i:1541496556;s:37:"./oscshop/admin/view/public/base.html";i:1512008342;}*/ ?>
<!DOCTYPE html>
<html lang="en">
	<head>
...
<table class="table table-striped table-bordered table-hover search-form">
	<thead>
		...
		<th><input name="code" id="code" type="text" value="" placeholder="输入优惠券码" /></th>		
		<!--<th><input name="user_name" type="text" placeholder="输入起始时间" value="<?php echo input('param.start_time'); ?>" /></th>		
		<th><input name="user_name" type="text" placeholder="输入截止时间" value="<?php echo input('param.end_time'); ?>" /></th>-->
		<!--<th>    				
			<select name="status">
				<option value="">-选择状态-</option>
				<option value="">以核销</option>
				<option value="">未核销</option>
				
			</select>
		</th>-->
		<th>
			
			<input id="send" type="submit" value="核销" class="btn btn-primary" />
			<input id="send2" type="submit" value="核销2" class="btn btn-primary" />
		</th>
	</thead>
</table>	
<div class="row">
	<div class="col-xs-12">	
		<div class="table-responsive">
			<table class="table table-striped table-bordered table-hover">
				...
				<tbody>
				<?php if(is_array($coupon) || $coupon instanceof \think\Collection || $coupon instanceof \think\Paginator): $i = 0; $__LIST__ = $coupon;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$co): $mod = ($i % 2 );++$i;?>
					<tr>
						<td>
							<?php echo $co['name']; ?>
						</td>
						<td>
							<?php echo $co['coupon_code']; ?>
						</td>
						<td>
							<?php echo $co['write_off_time']; ?>
						</td>
						<td>
							已使用
						</td>
					
						<!--<td colspan="20" class="page"><?php echo $coupon->render(); ?></td>-->
					</tr>
				<?php endforeach; endif; else: echo "" ;endif; ?>	
				</tbody>
				<tr>
...
alert(code);
	$.post(
		"<?php echo url('Writeoff/certificate'); ?>",
		{
			code:code
		},
		function(d){
...
alert(code);
	$.post(
		"<?php echo url('Writeoff/partnerwrite'); ?>",
		{
			code:code
		},
		function(d){
