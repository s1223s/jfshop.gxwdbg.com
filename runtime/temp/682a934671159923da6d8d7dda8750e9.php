<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:45:"./oscshop/mobile/view/agent_backend/edit.html";i:1512008342;s:37:"./oscshop/admin/view/public/base.html";i:1512008342;}*/ ?>
<!DOCTYPE html>
<html lang="en">
	<head>
...
<div class="row">
	<div class="col-xs-12">	
		<div class="form-horizontal">
			...
			<input name="agent_id" type="hidden" value="<?php echo \think\Request::instance()->param('id'); ?>" />
			
			<div class="form-group">
				<label class="col-sm-2 control-label no-padding-left"> 姓名</label>
			<div class="col-sm-10">
				<div class="clearfix">					
						<input class="form-control" name="name" type="text" value="<?php echo $agent['name']; ?>" /> 			
				</div>
			</div>
			</div>
			...
			<div class="col-sm-10">
				<div class="clearfix">					
						<input class="form-control" name="tel" type="text"  value="<?php echo $agent['tel']; ?>"  /> 			
				</div>
			</div>
			</div>
			<div class="space-4"></div>
			...
			<div class="form-group">
				<label class="col-sm-2 control-label no-padding-left"> * 代理级别 </label>
			<div class="col-sm-10">
				<div class="clearfix">
					<select  class="form-control" name="agent_level">
						<?php if(is_array($level) || $level instanceof \think\Collection || $level instanceof \think\Paginator): $i = 0; $__LIST__ = $level;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>							
							<option value="<?php echo $v['al_id']; ?>" <?php if(isset($agent)&&($v['al_id'] == $agent['agent_level'])): ?>selected="selected"<?php endif; ?>  ><?php echo $v['name']; ?></option>								
						<?php endforeach; endif; else: echo "" ;endif; ?>
					</select>
				</div>
			</div>
			...
			<div class="col-sm-10">
				<div class="clearfix">
					<label class="radio-inline">
						<input name="status" type="radio" checked="checked" value="1" /> 开启
					</label>
					<label class="radio-inline">
...
<script>
$(function(){	
	Oscshop.setValue("status", <?php echo (isset($agent['status']) && ($agent['status'] !== '')?$agent['status']:1); ?>);	
	
	var back_index="<?php echo url('AgentBackend/agent_list'); ?>";
	
	$('#send').click(function(){
		$.post(
			"<?php echo url('AgentBackend/edit_agent'); ?>",
