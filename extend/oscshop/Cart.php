...
class Cart{
			...
			foreach ($cart_list as $k => $cart) {
				...
				if($goods){
					...
					$goods_data[] = array(
						...
						'coupon'					=> $goods['coupon_type'],
						'merchant'					=> $goods['merchantNo'],
						'stock'                     => $stock,
						'option'                    => $option_data,
					);
	...
	public function has_shipping($uid,$type='money') {
		...
		//echo $coupon;
		if($coupon>1){
			return ['error'=>$no_shipping_goods.'同时只能购买一种券'];
		}
		/*elseif($coupon == 1&&$product['quantity']>1){
			return ['error'=>$no_shipping_goods.'一次只能购买一张券'];
		}*/
