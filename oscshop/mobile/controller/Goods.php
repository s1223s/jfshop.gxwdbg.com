...
class Goods extends MobileBase{
    ...
    function detail(){
		...
		//$t = Db::name('goods_discountms')->where('goods_id',(int)input('param.id'))->order('quantity ASC')->select();
		...
		$this->assign('discountms',$list['discountms']);
		$this->assign('mobile_description',$list['mobile_description']);		
		$this->assign('merchantNo',$list['goods']['merchantNo']);
		$this->assign('sales_volume',$list['goods']['sales_volume']);
